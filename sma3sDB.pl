...
############################################################################
## Connect to postgresql database containing preloaded .annot information ##
############################################################################
use DBI;
use strict;
...
"GO:0048646"=>"P:anatomical structure formation involved in morphogenesis", "GO:0048856"=>"P:anatomical structure development", "GO:0048870"=>"P:cell motility", "GO:0050877"=>"P:neurological system process", "GO:0051186"=>"P:cofactor metabolic process", "GO:0051276"=>"P:chromosome organization", "GO:0051301"=>"P:cell division", "GO:0051604"=>"P:protein maturation", "GO:0055085"=>"P:transmembrane transport", "GO:0061024"=>"P:membrane organization", "GO:0065003"=>"P:macromolecular complex assembly", "GO:0071554"=>"P:cell wall organization or biogenesis", "GO:0071941"=>"P:nitrogen cycle metabolic process");
...
print "Reading annotations from UniProt reference filwee\n";

################################################################################################################
#        BLUID ANN, N_ANNOT & FREQ FROM DB                                                                     #
################################################################################################################
### Retrieving Annotations for each protein
sub extract_ann {
	my ($extends, $quality, $goslim, $protein_id) = @_;

	my $query;
	my $from_where= "FROM protein as pr WHERE pr.id=?";	
	my $goslim_subquery_column = "
  ||E'\t'\|| COALESCE(
  (select string_agg(go_slim,';') 
   from protein_go_slim pgoslim 
   where pgoslim.protein_id=pr.id),'')";

   my $no_extends_no_quality = "
SELECT 
   -- SCORE
   COALESCE(
  (select score1::text||','||score2::text
   from protein_score pscore 
   where pscore.protein_id=pr.id),'') ||E'\t'||
   
  -- GENENAME e
  COALESCE(
  (select string_agg(gene,';') 
   from protein_gene pgene 
   where pgene.protein_id=pr.id),'') ||E'\t'||
   
  -- DESCRIPTION e
  COALESCE(
  (select string_agg(protein_description,';') 
   from protein_description pdescription
   where pdescription.id=pr.id),'') ||E'\t'||
  
  -- ENZYME e
  COALESCE(
  (select string_agg(enzyme,';') 
   from protein_enzyme penzyme 
   where penzyme.protein_id=pr.id),'') || E'\t' ||
   
  -- GO e iea
  COALESCE(
  (select string_agg(go,';')
	from (  select go
			from protein_goc pgoc 
			where pgoc.protein_id=pr.id
			UNION
			select go
			from protein_gof pgof 
			where pgof.protein_id=pr.id
			UNION
			select go
			from protein_gop pgop 
			where pgop.protein_id=pr.id
		 ) gounion ) ,'')||E'\t'||
  
  -- KEYWORD e
  COALESCE(
  (select string_agg(keyword,';') 
   from protein_keyword pkeyword 
   where pkeyword.protein_id=pr.id),'') ||E'\t'||
   
  -- PATHWAY e
  COALESCE(
  (select string_agg(pathway,';') 
   from protein_pathway ppathway 
   where ppathway.protein_id=pr.id),'')
   ";

my $extends_no_quality="
SELECT 
   -- SCORE
   COALESCE(
  (select score1::text||','||score2::text
  ...
  COALESCE(
  (select string_agg(go,';')
	from (  select SPLIT_PART(go,'GO:',2)||':C:'||(select go_name from goc where goc.go=pgoc.go)
			from protein_goc pgoc 
			where pgoc.protein_id=pr.id
			UNION
			select SPLIT_PART(go,'GO:',2)||':F:'||(select go_name from gof where gof.go=pgof.go)
			from protein_gof pgof 
			where pgof.protein_id=pr.id
			UNION
			select SPLIT_PART(go,'GO:',2)||':P:'||(select go_name from gop where gop.go=pgop.go)
			from protein_gop pgop 
			where pgop.protein_id=pr.id
		 ) gounion ) ,'')||E'\t'||
 ...
 SELECT
  ...
  (select string_agg(gene,';') 
   from protein_gene pgene 
   where pgene.protein_id=pr.id
   AND pgene.quality = TRUE),'') ||E'\t'||
   
  -- DESCRIPTION e
  COALESCE(
  (select string_agg(protein_description,';') 
   from protein_description pdescription
   where pdescription.id=pr.id
   AND pdescription.quality = TRUE),'') ||E'\t'||
  
  -- ENZYME e
  COALESCE(
  (select string_agg(enzyme,';') 
   from protein_enzyme penzyme 
   where penzyme.protein_id=pr.id
   AND penzyme.quality = TRUE),'') || E'\t' ||
  ...
  (select string_agg(go,';')
	from (  select go
			from protein_goc pgoc 
			where pgoc.protein_id=pr.id 
            AND pgoc.evidence_code  NOT LIKE 'IEA'
			UNION
			select go
			from protein_gof pgof 
			where pgof.protein_id=pr.id
			AND pgof.evidence_code  NOT LIKE 'IEA'
			UNION
			select go
			from protein_gop pgop 
			where pgop.protein_id=pr.id
			AND pgop.evidence_code  NOT LIKE 'IEA'
		 ) gounion ) ,'')||E'\t'||
  ...
  (select string_agg(keyword,';') 
   from protein_keyword pkeyword 
   where pkeyword.protein_id=pr.id
   AND pkeyword.quality = TRUE),'') ||E'\t'||
  ...
  (select string_agg(pathway,';') 
   from protein_pathway ppathway 
   where ppathway.protein_id=pr.id
   AND ppathway.quality = TRUE),'')
";

my $extends_and_quality="
SELECT 
   -- SCORE
   COALESCE(
  (select score1::text||','||score2::text
...
# Select the query depending on user parameters
	if($extends){
		if($quality){
			$query=$extends_and_quality;
		} else{
			$query=$extends_no_quality;
		}
	} else {
		if($quality){
			$quality = $no_extends_and_quality;
		} else{
			$query = $no_extends_no_quality;
		}
	}

    if($goslim){
        $query=$query.$goslim_subquery_column;
    }
    
	my $stmt = $query.$from_where;
	my $sth = $dbh->prepare( $stmt );
	my $rv = $sth->execute($protein_id) or die $DBI::errstr;
	if($rv < 0) {
	   print $DBI::errstr;
	}
	my @annot_output_array = $sth->fetchrow_array();
	my $annot_output = $annot_output_array[0];
...
my %N_ANNOT;
print "## GENENAME";
my $stmt = "select count(*) from protein_gene;";
my $stmt_quality = "select count(*) from protein_gene where quality= TRUE;";
if($QUALITY){
    $stmt = $stmt_quality
}
my $sth = $dbh->prepare( $stmt );
...
my @n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"GENENAME"}=$n_annot_array[0];
print $N_ANNOT{"GENENAME"};

## DESCRIPTION
$stmt = "select count(*) from protein_description;";
$stmt_quality = "select count(*) from protein_description where quality= TRUE;";
if($QUALITY){
    $stmt = $stmt_quality
}
...
@n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"DESCRIPTION"}=$n_annot_array[0];

## ENZYME
$stmt = "select count(*) from protein_enzyme;";
$stmt_quality = "select count(*) from protein_enzyme where quality= TRUE;";
if($QUALITY){
    $stmt = $stmt_quality
}
$sth = $dbh->prepare( $stmt );
...
@n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"ENZYME"}=$n_annot_array[0];

## GO
$stmt = "select (select count(*) from protein_goc)+(select count(*) from protein_gof)+(select count(*) from protein_gop);";
$stmt_quality = "select (select count(*) from protein_goc where evidence_code NOT LIKE = 'IEA')+(select count(*) from protein_gof where evidence_code NOT LIKE = 'IEA')+(select count(*) from protein_gop where evidence_code NOT LIKE = 'IEA');";
if($QUALITY){
    $stmt = $stmt_quality;
}
$sth = $dbh->prepare( $stmt );
...
@n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"GO"}=$n_annot_array[0];

## PATHWAY
$stmt = "select count(*) from protein_pathway;";
$stmt_quality = "select count(*) from protein_pathway where quality= TRUE;";
if($QUALITY){
    $stmt = $stmt_quality;
}
$sth = $dbh->prepare( $stmt );
...
@n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"PATHWAY"}=$n_annot_array[0];

## KEYWORD
$stmt = "select count(*) from protein_keyword;";
$stmt_quality = "select count(*) from protein_keyword where quality= TRUE;";
if($QUALITY){
    $stmt = $stmt_quality;
}
$sth = $dbh->prepare( $stmt );
...
@n_annot_array=$sth->fetchrow_array();
$N_ANNOT{"KEYWORD"}=$n_annot_array[0];
#############################################################
#############################################################
...
## GENENAME
$stmt = "select gene, count(gene) from protein_gene group by gene;";
$stmt_quality = "select gene, count(gene) from protein_gene where quality = TRUE group by gene;";
if($QUALITY){
    $stmt = $stmt_quality
}
$sth = $dbh->prepare( $stmt );
...
while(my @row = $sth->fetchrow_array()) {
    $FREQ{"GENENAME"}{$row[0]}=$row[1];
}

## DESCRIPTION
$stmt = "select description, count(description) from protein_description group by description;";
$stmt_quality = "select description, count(description) from protein_description where quality = TRUE group by description;";
if($QUALITY){
    $stmt = $stmt_quality
}
$sth = $dbh->prepare( $stmt );
...
while(my @row = $sth->fetchrow_array()) {
    $FREQ{"DESCRIPTION"}{$row[0]}=$row[1];
}

## ENZYME
$stmt = "select enzyme, count(enzyme) from protein_enzyme group by enzyme;";
$stmt_quality = "select enzyme, count(enzyme) from protein_enzyme where quality = TRUE group by enzyme;";
if($QUALITY){
    $stmt = $stmt_quality
}
$sth = $dbh->prepare( $stmt );
...
while(my @row = $sth->fetchrow_array()) {
    $FREQ{"ENZYME"}{$row[0]}=$row[1];
}

## GO F
if($EXTEND){
    $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':F:'||(SELECT go_name FROM gof WHERE gof.go=pgof.go), count(*) from protein_gof pgof GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':F:'||(SELECT go_name FROM gof WHERE gof.go=pgof.go), count(*) FROM protein_gof pgof WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";  
    }
}else{
    my $stmt = "SELECT go, count(*) FROM protein_gof GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT go, count(*) FROM protein_gof WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";
    }
}
...
## GO P
if($EXTEND){
    $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':P:'||(SELECT go_name FROM gop WHERE gop.go=pgop.go), count(*) from protein_gop pgop GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':P:'||(SELECT go_name FROM gop WHERE gop.go=pgop.go), count(*) FROM protein_gop pgop WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";  
    }
}else{
    my $stmt = "SELECT go, count(*) FROM protein_gop GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT go, count(*) FROM protein_gop WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";
    }
}
$sth = $dbh->prepare( $stmt );
$rv = $sth->execute() or die $DBI::errstr;
...
## GO C
if($EXTEND){
    $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':C:'||(SELECT go_name FROM goc WHERE goc.go=pgoc.go), count(*) from protein_goc pgoc GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT SPLIT_PART(go,'GO:',2)||':C:'||(SELECT go_name FROM goc WHERE goc.go=pgoc.go), count(*) FROM protein_goc pgoc WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";  
    }
}else{
    my $stmt = "SELECT go, count(*) FROM protein_goc GROUP BY go;";
    if($QUALITY){
        $stmt = "SELECT go, count(*) FROM protein_goc WHERE evidence_code NOT LIKE 'IEA' GROUP BY go;";
    }
}
$sth = $dbh->prepare( $stmt );
$rv = $sth->execute() or die $DBI::errstr;
...
## PATHWAY
$stmt = "select pathway, count(pathway) from protein_pathway group by pathway;";
my $stmt_quality = "select pathway, count(pathway) from protein_pathway where quality= TRUE group by pathway;";
if($QUALITY){
    $stmt = $stmt_quality
}
$sth = $dbh->prepare( $stmt );
...
while(my @row = $sth->fetchrow_array()) {
    $FREQ{"PATHWAY"}{$row[0]}=$row[1];
}

## KEYWORD
$stmt = "select keyword, count(keyword) from protein_keyword group by keyword;";
$stmt_quality = "select keyword, count(keyword) from protein_keyword  where quality= TRUE group by keyword;";
if($QUALITY){
    $stmt = $stmt_quality;
}
$sth = $dbh->prepare( $stmt );
...
QUERY: foreach my $q (@queries) { # Go through the FIRST BLAST
  ...
  if ($ANNOTATOR =~ /1/) {
    ...
    if ($id_hsp >= $ID_UNIPROT && $qc_s >= $COV_UNIPROT && $pv_hsp <= $PV) { # alignment: 90% id + 90% qcoverage + pvalue
      ...
      my ($s, $gn1, $de1, @remaining) = split/\t/, extract_ann($EXTEND,$QUALITY,$GOSLIM,$name2);
      (@s1) = split/,/, $s; # scores
      ...
      foreach my $cluster (@clusters) {
        ...
        foreach my $id (@ids) { # Ids in a cluster
          for (my $i = 0; $i <= $#TYPES; $i++) { # annotation columns
            my $type = $TYPES[$i];
            my ($terms) = (split /\t/, extract_ann($EXTEND,$QUALITY,$GOSLIM,$id))[$i+1];
...
##! BEGIN quitar cosas no base de datos
...
#   return;
# }
##! END quitar cosas no base de datos
...
Please run as: $0 -i fasta_input_file -d target_database.fasta
 ...
 argument without values (they are activated if you give the argument)
  ...
  DEPRECATED -uniprot  if you use a uniprot database (with .dat extension) instead of the default uniref database
